-- Live-DB smoke test for alucard.lua's Postgres write paths (mirrors
-- nexus/sapphire/symphony's db_smoke_test.lua pattern). Runs against the
-- real discord_music_alucard database (already-migrated, shared with the
...
-- touch real guild data (e.g. the pre-existing swarm_health/alucard_metrics
-- row for guild 1247394560007471134).
--
-- Run with: ALUCARD_DRY_RUN=test ALUCARD_DB_PORT=5432 luajit db_smoke_test.lua

package.path = "./lib/?.lua;./lib/?/init.lua;" .. package.path

-- Expects ALUCARD_DRY_RUN=test in the environment already (set by the caller,
-- e.g. `ALUCARD_DRY_RUN=test ALUCARD_DB_PORT=5432 luajit db_smoke_test.lua`).
local A = dofile("alucard.lua")
...
A.q("UPDATE alucard_guild_settings SET volume = %s, loop_mode = %s WHERE guild_id = %s", 77, "song", TEST_GUILD)
...
local rows = A.q("SELECT video_url, title FROM alucard_queue WHERE guild_id = %s AND bot_name = 'alucard' ORDER BY id ASC", TEST_GUILD)
...
local backup_count = A.q1("SELECT COUNT(*)::int AS n FROM alucard_queue_backup WHERE guild_id = %s AND bot_name = 'alucard'", TEST_GUILD)
...
A.q("DELETE FROM alucard_queue WHERE guild_id = %s AND bot_name = 'alucard'", TEST_GUILD)
...
local backup_after = A.q1("SELECT COUNT(*)::int AS n FROM alucard_queue_backup WHERE guild_id = %s AND bot_name = 'alucard'", TEST_GUILD)
...
-- insert, per the live schema introspected via `\d alucard_track_intelligence`
-- and `\d alucard_user_track_affinity` -- both confirmed NOT NULL with no
...
local ti = A.q1("SELECT queued_count, play_count, finish_count, like_count, dislike_count, skip_count, total_listen_seconds FROM alucard_track_intelligence WHERE guild_id = %s AND url_key = %s", TEST_GUILD, key_a)
...
local aff = A.q1("SELECT queued_count, play_count, finish_count, skip_count, like_count, dislike_count, score FROM alucard_user_track_affinity WHERE guild_id = %s AND user_id = %s AND url_key = %s", TEST_GUILD, TEST_USER, key_a)
...
local ti2 = A.q1("SELECT queued_count FROM alucard_track_intelligence WHERE guild_id = %s AND url_key = %s", TEST_GUILD, A.track_key("https://example.com/a", "Track A"))
...
A.q([[INSERT INTO alucard_playback_state (guild_id, bot_name, channel_id, video_url, position_seconds, is_playing, is_paused, title, play_session_key, track_uid)
      VALUES (%s, 'alucard', %s, %s, 42, true, false, %s, %s, %s)
...
local ps = A.q1("SELECT position_seconds, is_playing FROM alucard_playback_state WHERE guild_id = %s AND bot_name = 'alucard'", TEST_GUILD)
...
ps = A.q1("SELECT video_url, is_playing, position_seconds FROM alucard_playback_state WHERE guild_id = %s AND bot_name = 'alucard'", TEST_GUILD)
...
-- `\d alucard_voice_state`: reconnect_attempts is NOT NULL, no default)
...
local vs = A.q1("SELECT connected_channel_id::text AS connected_channel_id, desired_connected, reconnect_attempts FROM alucard_voice_state WHERE guild_id = %s AND bot_name = 'alucard'", TEST_GUILD)
...
vs = A.q1("SELECT connected_channel_id, desired_connected FROM alucard_voice_state WHERE guild_id = %s AND bot_name = 'alucard'", TEST_GUILD)
check("voice_state disconnected row correct", vs and vs.connected_channel_id == nil and vs.desired_connected == false, tostring(vs))

-- heartbeat_tick / alucard_metrics (the %s-placeholder-count vs args-supplied
...
local metrics = A.q1("SELECT queue_count, is_playing_db, is_paused_db, position_seconds, duration_seconds FROM alucard_metrics WHERE guild_id = %s AND bot_name = 'alucard'", TEST_GUILD)
check("alucard_metrics row written", metrics ~= nil, tostring(metrics))
check("alucard_metrics is_paused_db is boolean false, not a position/duration value", metrics and metrics.is_paused_db == false, metrics and tostring(metrics.is_paused_db))
check("alucard_metrics position_seconds is a plausible position (current_position, not duration/title)", metrics and tonumber(metrics.position_seconds) ~= nil and tonumber(metrics.position_seconds) < 300, metrics and tostring(metrics.position_seconds))
check("alucard_metrics duration_seconds == 200 (not swapped with position)", metrics and tonumber(metrics.duration_seconds) == 200, metrics and tostring(metrics.duration_seconds))
...
local pst = A.q1("SELECT position_seconds FROM alucard_playback_state WHERE guild_id = %s AND bot_name = 'alucard'", TEST_GUILD)
...
local ee = A.q1("SELECT title, error_type FROM alucard_error_events WHERE guild_id = %s AND error_type = 'smoke_test' ORDER BY created_at DESC LIMIT 1", TEST_GUILD)
...
A.q("DELETE FROM alucard_queue WHERE guild_id = %s AND bot_name = 'alucard'", TEST_GUILD)
A.q("DELETE FROM alucard_queue_backup WHERE guild_id = %s AND bot_name = 'alucard'", TEST_GUILD)
A.q("DELETE FROM alucard_playback_state WHERE guild_id = %s AND bot_name = 'alucard'", TEST_GUILD)
A.q("DELETE FROM alucard_guild_settings WHERE guild_id = %s", TEST_GUILD)
A.q("DELETE FROM alucard_swarm_toggles WHERE guild_id = %s", TEST_GUILD)
A.q("DELETE FROM alucard_track_intelligence WHERE guild_id = %s", TEST_GUILD)
A.q("DELETE FROM alucard_user_track_affinity WHERE guild_id = %s", TEST_GUILD)
A.q("DELETE FROM alucard_voice_state WHERE guild_id = %s AND bot_name = 'alucard'", TEST_GUILD)
A.q("DELETE FROM alucard_error_events WHERE guild_id = %s", TEST_GUILD)
A.q("DELETE FROM alucard_metrics WHERE guild_id = %s AND bot_name = 'alucard'", TEST_GUILD)

local leftover = A.q1([[
  SELECT
    (SELECT COUNT(*) FROM alucard_queue WHERE guild_id = %s) +
    (SELECT COUNT(*) FROM alucard_queue_backup WHERE guild_id = %s) +
    (SELECT COUNT(*) FROM alucard_playback_state WHERE guild_id = %s) +
    (SELECT COUNT(*) FROM alucard_guild_settings WHERE guild_id = %s) +
    (SELECT COUNT(*) FROM alucard_track_intelligence WHERE guild_id = %s) +
    (SELECT COUNT(*) FROM alucard_user_track_affinity WHERE guild_id = %s) +
    (SELECT COUNT(*) FROM alucard_voice_state WHERE guild_id = %s) +
    (SELECT COUNT(*) FROM alucard_error_events WHERE guild_id = %s) +
    (SELECT COUNT(*) FROM alucard_metrics WHERE guild_id = %s) AS n
...
local real_guild = A.q1("SELECT is_playing_db FROM alucard_metrics WHERE guild_id = %s AND bot_name = 'alucard'", "1247394560007471134")
check("pre-existing real guild_id 1247394560007471134 row in alucard_metrics untouched", real_guild ~= nil, tostring(real_guild))
