...
![Alucard bot icon](assets/bot-icon.png)

Alucard is one node in the 13-bot Discord music fleet. Named after Castlevania's dhampir prince — at home running the night shift. It owns its queue tables, reports playback state to the shared swarm database, and is controlled directly from Discord, Aria, and SwarmPanel.

## Identity

- Discord display name: **GWS Music Bot (Alucard)**
- Internal node name: `alucard`
- Command namespace: `/alucard_main_*`
...
All commands are namespaced as `/alucard_main_<command>`.
...
- Node: `alucard`
- Database schema: `discord_music_alucard`
