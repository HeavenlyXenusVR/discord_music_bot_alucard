...
function Pg:ensure()
  ...
  -- BUGFIX (see final report): Discord snowflake IDs (guild/user/channel/role) live in
  -- BIGINT columns and routinely exceed 2^53, the largest integer a Lua double can represent
  -- exactly. pgmoon's default OID 20 (int8) deserializer runs every bigint value through
  -- tonumber(), which silently corrupts them, e.g. 1304564041863266347 comes back as
  -- 1.3045640418633e+18. Force bigint columns to come back as the raw wire-format string
  -- instead, which preserves full precision. This is safe to hand straight back into another
  -- bigint column or WHERE clause: Postgres implicitly casts untyped string literals to the
  -- target numeric type. Verified live against discord_music_alucard.
...
function Pg:query(sql, ...)
  ...
  -- select('#', ...) (not #args) is required: a plain "{...}" table stops at the first
  -- embedded/trailing nil, silently dropping trailing NULL params and shifting the %s
  -- placeholders against the remaining args. See lua-shared canonical pg.lua.
  local n = select('#', ...)
  if n > 0 then
    local args = { ... }
    for i = 1, n do
