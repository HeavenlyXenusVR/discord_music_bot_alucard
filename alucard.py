...
async def _process_queue_inner(guild, channel_id, start_position=0, *, allow_recovery_restore=False):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("SELECT volume, loop_mode, filter_mode, transition_mode, custom_speed, custom_pitch, custom_modifiers_left, stay_in_vc FROM alucard_guild_settings WHERE guild_id = %s", (guild.id,))
                res = await cur.fetchone()
...
@bot.tree.command(name="alucard_main_fade", description="Enable or disable smooth fade transitions between tracks for softer playback changes.")
...
async def toggle_fade(interaction: discord.Interaction, mode: str):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("UPDATE alucard_guild_settings SET transition_mode = %s WHERE guild_id = %s", (mode, interaction.guild.id))
