...
async def build_autodj_query(cur, guild_id):
    await cur.execute("SELECT title FROM alucard_history WHERE guild_id = %s ORDER BY played_at DESC LIMIT 12", (guild_id,))
...
async def maybe_enqueue_autodj(cur, guild, channel_id):
    ...
    try:
        ...
        history_titles = set()
        await cur.execute("SELECT title FROM alucard_history WHERE guild_id = %s ORDER BY played_at DESC LIMIT 18", (guild.id,))
            ...
            async with pool.acquire() as conn:
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    for g in guilds:
                        ...
                        await cur.execute("SELECT COUNT(*) AS total FROM alucard_queue WHERE guild_id = %s AND bot_name = 'alucard'", (g.id,))
                        queue_row = await cur.fetchone() or {}
                        await cur.execute("SELECT COUNT(*) AS total FROM alucard_queue_backup WHERE guild_id = %s AND bot_name = 'alucard'", (g.id,))
                        backup_row = await cur.fetchone() or {}
                        await cur.execute("SELECT is_playing, is_paused, position_seconds FROM alucard_playback_state WHERE guild_id = %s AND bot_name = 'alucard' LIMIT 1", (g.id,))
