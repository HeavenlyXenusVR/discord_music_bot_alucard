    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                await cur.execute("SELECT id, video_url, title FROM alucard_queue WHERE guild_id = %s AND bot_name = 'alucard' ORDER BY id ASC LIMIT %s", (interaction.guild.id, index-1))
                rows = await cur.fetchall()
                for r in rows:
                    await cur.execute("DELETE FROM alucard_queue WHERE id = %s AND guild_id = %s AND bot_name = 'alucard'", (r[0], interaction.guild.id))
                    await cur.execute(
                        "DELETE FROM alucard_queue_backup WHERE video_url = %s AND title = %s AND guild_id = %s AND bot_name = 'alucard' LIMIT 1",
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                for url, title in q:
                    await cur.execute("INSERT INTO alucard_user_playlists (user_id, playlist_name, video_url, title) VALUES (%s, %s, %s, %s)", (interaction.user.id, name, url, title))
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                res = await cur.fetchone()
                if res and res[0] != 'none': return await interaction.response.send_message(embed=discord.Embed(description="❌ **Conflict:** Disable standard Filters via `/alucard_main_filter none` first.", color=discord.Color.red()), ephemeral=True)
